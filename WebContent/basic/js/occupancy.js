...
var roomGrid=null;
var cbx_pancyState=null;
var cbx_param8=null;//缴费状态
var cbx_payState=null;
var cbx_param2=null;//房间状态
var cbx_paidType=null;
...
var occupancy={
	 ...
	 */
	init:function(){
		dataGrid=$('#occupancyGrid').datagrid({
		    loadMsg:"正在加载..",
		    pagination:true,
		    rownumbers:true,
		    pagePosition:'bottom',
		    ...
		    //view:'暂无数据',
		    columns:[[
				{field:'custName',title:'客户名称',width:100,formatter:function(val,row,index){
					if(common.isNotBlank(row.customer)){
						return row.customer.custName;
					}
				}},
				{field:'cardNumber',title:'身份证',width:150,formatter:function(val,row,index){
					if(common.isNotBlank(row.customer)){
						return row.customer.cardNumber;
					}
				}},
				{field:'roomNumber',title:'房间号',width:100,formatter:function(val,row,index){
					if(common.isNotBlank(row.roomInfo)){
						return row.roomInfo.roomNumber;
					}
				}},
				{field:'roomPrice',title:'标准价(元)',width:80,formatter:function(val,row,index){
					if(common.isNotBlank(row.roomInfo)){
						return row.roomInfo.roomPrice;
					}
				}},
				{field:'amountOwe',title:'押金(元)',width:80,formatter:function(val,row,index){
					return row.amountOwe;
				}},
				{field:'prepaidAmount',title:'应收金额',width:80},
				{field:'amountCollect',title:'实收金额',width:100},
				{field:'checkInDateView',title:'入住时间',width:120},
				{field:'checkOutDateView',title:'退房时间',width:120},
				{field:'prepaidNumber',title:'入住天数',width:60},
				{field:'payState',title:'缴费状态',width:100,formatter:function(v,r,i){
					//1.预收款、2.已收费、3.已退款
					if(common.isNotBlank(r)){
						if(r.payState=="1"){
							...
							return "已收费";
						}else if(r.payState=="3"){
							return "已退款";
						}
					}
				}},
				{field:'paidType',title:'付款方式',width:100},
				{field:'pancyState',title:'订单状态',width:100,formatter:function(v,r,i){
					//订单状态: 1：入住,2：预订，3.换房，4：退房,5：取消,0：结束
					if(common.isNotBlank(r)){
						if(r.pancyState=="1"){
							return "入住";
						}else if(r.pancyState=="2"){
							return "预订";
						}else if(r.pancyState=="3"){
							return "换房";
						}else if(r.pancyState=="4"){
							return "退房";
						}else if(r.pancyState=="5"){
							return "取消";
						}else if(r.pancyState=="0"){
							return "结束";
						}
					}
				}},
				{field:'createByName',title:'创建人',width:100},
				{field:'createDateView',title:'创建日期',width:120},
				{field:'opt',title:'操作',formatter:function(val,rows,index){
					var btn='<button class="layui-btn layui-btn-sm" onclick="occupancy.tab(\'see\',\''+rows.pancyId+'\')"><i class="layui-icon">&#xe615;</i>查看</button>';
					//订单状态: 1：入住,2：预订，3.换房，4：退房,5：取消,0：结束
					if(common.getstrlength(rows.pancyState) >0){
						//预定
						if(rows.pancyState=='2'){
							btn+='<button class="layui-btn layui-btn-normal layui-btn-sm" onclick="occupancy.editState(\'rz\',\''+index+'\')"><i class="layui-icon">&#xe642;</i>入住</button>'
							btn+='<button class="layui-btn layui-btn-normal layui-btn-sm" onclick="occupancy.editState(\'qx\',\''+index+'\')"><i class="layui-icon">&#xe642;</i>取消</button>'
							btn+='<button class="layui-btn layui-btn-normal layui-btn-sm" onclick="occupancy.editState(\'hf\',\''+index+'\')"><i class="layui-icon">&#xe642;</i>换房</button>'
						}else if(rows.pancyState=='4' || rows.pancyState=='5'){
							btn+='<button class="layui-btn layui-btn-danger layui-btn-sm" onclick="occupancy.tab(\'delete\',\''+rows.pancyId+'\')"><i class="layui-icon">&#xe640;</i>删除</button>'
						}else{//1 3
							btn+='<button class="layui-btn layui-btn-normal layui-btn-sm" onclick="occupancy.editState(\'hf\',\''+index+'\')"><i class="layui-icon">&#xe642;</i>换房</button>'
							btn+='<button class="layui-btn layui-btn-normal layui-btn-sm" onclick="occupancy.editState(\'tf\',\''+index+'\')"><i class="layui-icon">&#xe642;</i>退房</button>'
						}
					}
				    return btn;
				}}
		    ]],
		    //view: occupancy.myview,
		...
		cbx_pancyState=new SelectBox("pancyState",{
			text_field:'dicName',
			value_field:'dicCode',
			multiple:false,
			width:'100%',
			dropWidth:'200px',
			//onChange:occupancy.chPancyState
		});
		
		var codeType='PAID_TYPE,PAY_STATE,PANCY_STATE';
		layer.load(3);
		...
		cbx_pancyState.bind([{'dicName':'入住','dicCode':'1'},{'dicName':'预定','dicCode':'2'}]);
		...
		custDataGrid=$('#customerGrid').datagrid({
		    ...
		    columns:[[
				...
				{field:'cardNumber',title:'身份证',width:100},
		...
		laydate.render({ 
		  ...
		  type: 'datetime', //默认，可不填
		  format:'yyyy-MM-dd HH:mm', //可任意组合
		  value:common._addDate(1)+" 12:00",
		  done: function(value, date, endDate){
		   var checkInDate=$("#checkInDate").val();
		   //var checkInDate=$("#checkOutDate").val();
		   if(common.isNotBlank(checkInDate)){
			   if(!common.compareDate(checkInDate,value)){
				   layer.layer("入住日期不能大于退房日期",{icon:2},function(index){
					   layer.close(index);
					   $("#checkInDate").click();
				   });
			   }else{
				   var flag=common.DateDiff(checkInDate,value);
				   if(flag){
					   $("#prepaidNumber").val(flag);
					   $("#prepaidAmount").val($("#roomPrice").val()*flag);
					   $("#amountCollect").val($("#roomPrice").val()*flag);
				   }
			   }
		   }
		  }
		...
		$('#cardNumber').bind('input propertychange', function(){
			...
			occupancy.calculation(cardNumber);
		});
	},
	...
	//入住状态
	chPancyState:function(val,text){
		/*if(val=='3'){
			var str='<td class="width-15 active1"><label class="pull-right">预定时间:</label></td>';
			str+='<td class="width-35"><input type="text" id="predeterminedDate" class="layui-input" /></td>';
			str+='<td class="width-15 active1"><label class="pull-right">到点时间:</label></td>';
			str+='<td class="width-35"><input type="text" id="exCheckOutDate" class="layui-input" /></td>';
			$("#pancyStateDiv").html(str);
		}else{
			var str='<td class="width-15 active1"><label class="pull-right">入住时间:</label></td>';
			str+='<td class="width-35"><input type="text" id="checkInDate" class="layui-input" /></td>';
			str+='<td class="width-15 active1"><label class="pull-right">到点时间:</label></td>';
			str+='<td class="width-35"><input type="text" id="exCheckOutDate" class="layui-input" /></td>';
			$("#pancyStateDiv").html(str);
		}
		laydate.render({ 
			elem: '#predeterminedDate',
			type:'datetime',
			type: 'datetime', //默认，可不填
			format:'yyyy-MM-dd HH:mm' //可任意组合
		});*/
	...
	selectCust:function(){
		...
		layer.open({
			...
			area: [(common.getWinWidth()*0.5)+'px',(common.getWinHeight()*0.9)+'px'],
			...
			btn2: function(index, layero){
				common.resetContent("occupancyForm");
				layer.close(index);
			},
				...
				if(type=='change'){
					...
					laydate.render({ 
					  ...
					  value:common._addDate(1)+" 12:00",
					   ...
					   var checkInDate=$("#checkInDate5").val();
					   if(common.isNotBlank(checkInDate)){
						   if(!common.compareDate(checkInDate,value)){
							   layer.layer("入住日期不能大于退房日期",{icon:2},function(index){
								   layer.close(index);
							   ...
							   var flag=common.DateDiff(checkInDate,value);
							   if(flag){
								   $("#prepaidNumber5").val(flag);
								   $("#prepaidAmount5").val($("#roomPrice5").val()*flag);
								   $("#amountCollect5").val($("#roomPrice5").val()*flag);
							   }
						   }
					   }
					  }
					});
					$("#roomNumber5").val(row.roomNumber);
					$("#roomPrice5").val(row.roomPrice);
					$("#roomId5").val(row.roomId);
					var checkInDate=$("#checkInDate5").val();
					var checkOutDate=$("#checkOutDate5").val();
					var flag=common.DateDiff(checkInDate,checkOutDate);
				    if(flag){
					   $("#prepaidNumber5").val(flag);
					   $("#prepaidAmount5").val(($("#roomPrice5").val()*flag)+common._parseFloat($("#deposit5").val()));
					   $("#amountCollect5").val(($("#roomPrice5").val()*flag)+common._parseFloat($("#deposit5").val()));
				    }
				    ...
				    if(flag){
					   $("#prepaidNumber").val(flag);
					   $("#prepaidAmount").val(($("#roomPrice").val()*flag)+common._parseFloat($("#deposit").val()));
					   $("#amountCollect").val(($("#roomPrice").val()*flag)+common._parseFloat($("#deposit").val()));
				    }
					roomGrid.datagrid("uncheckAll");
					layer.close(index);
				}
	...
	loadGrid:function(params){
		dataGrid.datagrid({
			url:common.excuteUrl("occupancyController/findByPage.do"),
			queryParams:params,
			method:"post"
		});
	},
		...
		if(type=='tf'){//退房
			  ...
			  yes:function(index, layero){
				  ...
				  layer.open({
						...
						yes:function(index1, layero1){
							...
							layer.load(3);
							common.ajaxCall(common.excuteUrl("occupancyController/checkOutRoom.do"),obj,function(ret){
								layer.closeAll("loading");	
								if(ret.statusCode==0){
			...
			layer.confirm('确定要取消该订单吗？', {
			  ...
			  yes:function(index, layero){
				  layer.close(index);
				  layer.load(3);
				  common.ajaxCall(common.excuteUrl("occupancyController/cancelRoom.do"),{'pancyId':row.pancyId},function(ret){
					  layer.closeAll("loading");
					  layer.close(index);
					  if(ret.statusCode==0){
			  ...
			  yes:function(index, layero){
				  ...
				  layer.open({
						...
						yes:function(index1, layero1){
							...
							layer.load(3);
							common.ajaxCall(common.excuteUrl("occupancyController/changeRoom.do"),obj,function(ret){
								layer.closeAll("loading");
								if(ret.statusCode==0){
									layer.close(index1);
									layer.msg(ret.statusMessage,{icon: 1},function(index2,l){
			  ...
			  yes:function(index, layero){
				  ...
				  layer.open({
					...
					yes:function(index1, layero1){
						...
						layer.load(3);
						common.ajaxCall(common.excuteUrl("occupancyController/checkInRoom.do"),obj,function(ret){
							layer.closeAll("loading");
							if(ret.statusCode==0){
								layer.msg(ret.statusMessage,{icon: 1},function(index2,l){
									occupancy.loadGrid();
	...
	tab:function(type,id){
		if(type=='delete'){
			layer.confirm('确定要删除该条记录吗？', {
			  ...
			  yes:function(index, layero){
				  layer.load(3);
				  common.ajaxCall(common.excuteUrl("occupancyController/deletePancyById.do"),{'pancyId':id},function(ret){
					  layer.closeAll("loading");
					  if(ret.statusCode==0){
			...
			layer.open({
				...
				content:$("#occupancyFormEdit"),
				btn:['确定', '取消'],
				yes:function(index, layero){
					occupancy.saveOrUpdate(index);
					//layer.close(index);
				},
				btn2: function(index, layero){
					layer.close(index);
				...
				success: function(layero, index){
					occupancy.editFlag(type);
					//编辑、查看
					if(type=="edit" || type=="see"){
						layer.load(3);
						common.ajaxCall(common.excuteUrl("/occupancyController/queryAssoByPancyId.do"),{'pancyId':id},function(ret){
							layer.closeAll("loading");
							if(ret.statusCode==0){
								occupancy.bindValue(ret.result);
							}else{
								layer.alert(ret.statusMessage,{icon:2});
							}
						});
						...
						//获得最大序列号
						layer.load(3);
						common.ajaxCall(common.excuteUrl("/occupancyController/getPancyMaxOrderFlag.do"),{},function(ret){
							layer.closeAll("loading");
							if(ret.statusCode==0){
	...
	bindValue:function(data){
		...
		common.bindFormData(data,'#occupancyForm');//18320204217  18320204217abcd
		...
		cbx_pancyState.setValue(data.pancyState);
	...
	saveOrUpdate:function(index){
		...
		//订单form
		var obj=common.getFormData("#occupancyForm");
		obj.custId=custId;
		...
		if(common.isNotBlank(obj.payState) && common.getstrlength(obj.payState)>0 && obj.payState=='1'){
		...
		var url="";
		if(common.getstrlength(obj.pancyId)==0 || common.isNotBlank(obj.pancyId)){
			url=common.excuteUrl("occupancyController/insertPancy.do");
		}else{
			url=common.excuteUrl("occupancyController/updatePancySelective.do");
		}
		layer.load(3);
